...
export default function LimingPlanTable({ 
  ...
  // Seřadit aplikace podle sequence_order (které už respektuje rok + sezónu + pořadí)
  const sortedApplications = [...applications].sort((a, b) => {
    // Pokud mají sequence_order, použij ho (správné globální pořadí)
    if (a.sequence_order !== undefined && b.sequence_order !== undefined) {
      return a.sequence_order - b.sequence_order
    }
    // Fallback na ruční řazení (pro starší data bez sequence_order)
  ...
  useEffect(() => {
    ...
    // Najdeme vybraný produkt (může být změněný nebo původní)
    let selectedProduct = currentApp
    const dosePerHa = parseFloat(editData.dose_per_ha) || currentApp.dose_per_ha
    
    if (editData.product_id && editData.product_id !== currentApp.lime_product_id) {
      const newProduct = products.find(p => p.id === editData.product_id)
      if (newProduct) {
        selectedProduct = {
          ...currentApp,
          lime_product_id: newProduct.id,
          product_name: newProduct.name,
          cao_content: newProduct.cao_content,
          mgo_content: newProduct.mgo_content
        }
      }
    }
    
    const newWarnings: string[] = []
    const caoPerHa = dosePerHa * (selectedProduct.cao_content / 100)
    const mgoPerHa = dosePerHa * (selectedProduct.mgo_content / 100)
    ...
    const env = (selectedProduct.cao_content / 100) + ((selectedProduct.mgo_content / 100) * MGO_NEUTRALIZING_FACTOR)
    ...
    // Predikce Mg
    const mgChange = mgoPerHa > 0 ? (mgoPerHa * 1000 * 0.6 * 0.4 * 1000) / (10000 * 0.2 * 1400) : 0
    const mgAfter = (currentApp.mg_after || 100) + mgChange
    ...
    setCalculatedValues({
      dosePerHa: dosePerHa.toFixed(2),
      totalDose: totalDose.toFixed(1),
      caoPerHa: caoPerHa.toFixed(2),
      mgoPerHa: mgoPerHa.toFixed(2),
      phAfter: phAfter.toFixed(1),
      mgAfter: Math.round(mgAfter),
      productName: selectedProduct.product_name,
      caoContent: selectedProduct.cao_content,
      mgoContent: selectedProduct.mgo_content
    })
  }, [editData.dose_per_ha, editData.product_id, editingId, products, parcelArea, plan.soil_type, applications])
  ...
  function startEdit(app: LimingApplication) {
    ...
    setEditData({
      ...
      product_id: app.lime_product_id
    })
    setCalculatedValues({})
    
    // Načteme produkty pro select
  ...
  function handleProductChange(newProductId: string) {
    ...
    // Přepočítáme dávku aby se zachovalo stejné množství CaO
    const targetCao = currentApp.cao_per_ha
    const newDosePerHa = targetCao / (newProduct.cao_content / 100)
    
    // Aktualizujeme editData s novým produktem a přepočítanou dávkou
    setEditData({
      ...editData,
      product_id: newProductId,
      dose_per_ha: newDosePerHa
    })
  }
  ...
  async function handleSave(applicationId: string) {
    ...
    try {
      ...
      const updateData: any = {
        ...
        dose_per_ha: parseFloat(calculatedValues.dosePerHa),
        total_dose: parseFloat(calculatedValues.totalDose),
        cao_per_ha: parseFloat(calculatedValues.caoPerHa),
        mgo_per_ha: parseFloat(calculatedValues.mgoPerHa),
        ph_after: parseFloat(calculatedValues.phAfter),
        mg_after: calculatedValues.mgAfter
      }
      
      // Pokud se změnil produkt, přidáme i informace o produktu
      if (editData.product_id && editData.product_id !== currentApp.lime_product_id) {
        updateData.lime_product_id = editData.product_id
        updateData.product_name = calculatedValues.productName
        updateData.cao_content = calculatedValues.caoContent
        updateData.mgo_content = calculatedValues.mgoContent
      }
      ...
      // Backend automaticky provede kaskádový přepočet navazujících aplikací
        ...
        <table className="w-full" style={{ minWidth: '1200px' }}>
          <thead className="bg-gray-50 border-b">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 tracking-wider min-w-[200px]">
                Produkt
              </th>
              <th className="px-4 py-3 text-right text-xs font-semibold text-gray-700 tracking-wider min-w-[120px]">
                Dávka (t/ha)
              </th>
              <th className="px-4 py-3 text-right text-xs font-semibold text-gray-700 tracking-wider min-w-[100px]">
                Celkem (t)
              </th>
              <th className="px-4 py-3 text-right text-xs font-semibold text-gray-700 tracking-wider min-w-[100px]">
                CaO (t/ha)
              ...
              <th className="px-4 py-3 text-center text-xs font-semibold text-gray-700 tracking-wider min-w-[120px]">
                pH
              </th>
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 tracking-wider min-w-[140px]">
              ...
              <th className="px-4 py-3 text-right text-xs font-semibold text-gray-700 tracking-wider min-w-[140px] group relative">
          ...
          <tbody className="divide-y divide-gray-200">
            {sortedApplications.map((app, idx) => {
              ...
              // Výpočet acidifikace od předchozí aplikace
              // OPRAVA: Acidifikace se zobrazí, pokud je mezi aplikacemi časový odstup (yearsGap > 1)
              // pH drop se počítá z předchozího ph_after
              const previousApp = idx > 0 ? sortedApplications[idx - 1] : null
              const yearsGap = previousApp ? app.year - previousApp.year : 0
              
              // OPRAVA: Zobrazit acidifikaci pokud jsou mezi aplikacemi roky (yearsGap > 1)
              const showAcidification = idx > 0 && yearsGap > 1
              
              // Generování jednotlivých acidifikačních roků
              const acidificationYears: Array<{year: number, phStart: number, phEnd: number}> = []
              if (showAcidification && previousApp) {
                // ✅ FIXNÍ KONSTANTY pro roční pokles pH (konzistentní s backendem)
                ...
                for (let i = 1; i < yearsGap; i++) {
                  ...
                  acidificationYears.push({ year: yearNumber, phStart, phEnd })
                }
              }
              
              return (
                <React.Fragment key={app.id}>
                  {/* Acidifikační roky mezi aplikacemi */}
                  {acidificationYears.map((acidYear, acidIdx) => (
                    <tr key={`acid-${app.id}-${acidYear.year}`} className="bg-orange-50 border-l-4 border-orange-400">
                      <td className="liming-sticky-col liming-sticky-col-1 px-4 py-2.5 bg-orange-50">
                        <span className="font-semibold text-orange-800">{acidYear.year}</span>
                      </td>
                      <td className="liming-sticky-col liming-sticky-col-2 px-4 py-2.5 bg-orange-50">
                        <span className="text-orange-600 text-sm italic">—</span>
                      </td>
                      <td className="px-4 py-2.5" colSpan={5}>
                        <div className="flex items-center gap-2 text-sm">
                          <span className="text-orange-800 font-medium">⬇️ Přirozená acidifikace půdy</span>
                          <span className="text-xs text-orange-600 italic ml-2">
                            ({plan.soil_type === 'L' ? 'lehká' : plan.soil_type === 'S' ? 'střední' : 'těžká'} půda)
                          </span>
                        </div>
                      </td>
                      <td className="px-4 py-2.5 text-center">
                        <div className="flex items-center justify-center gap-1.5 text-orange-900 font-mono text-sm">
                          <span>{acidYear.phStart.toFixed(2)}</span>
                          <span className="text-orange-500">→</span>
                          <span className="font-semibold">{acidYear.phEnd.toFixed(2)}</span>
                          <span className="text-xs text-orange-600 ml-1">
                            ({(acidYear.phStart - acidYear.phEnd > 0 ? '-' : '+')}{Math.abs(acidYear.phStart - acidYear.phEnd).toFixed(2)})
                          </span>
                        </div>
                      </td>
                    ...
                    {isEditing ? (
                      <div>
                        <select
                          ...
                          value={editData.product_id || app.lime_product_id}
                        ...
                        {editData.product_id !== app.lime_product_id && (
                          <p className="text-xs text-green-600 mt-1">
                            ✓ Dávka přepočtena - můžete ji upravit
                          </p>
                        )}
                      </div>
                    ) : (
                    ...
                    {isEditing ? (
                      <div>
                        <input
                          type="number"
                          step="0.1"
                          className={`w-24 px-2 py-1 border rounded focus:ring-2 text-right ${
                            warnings.length > 0 ? 'border-red-500 focus:ring-red-500' : 'border-gray-300 focus:ring-green-500'
                          }`}
                          value={editData.dose_per_ha || ''}
                          onChange={(e) => setEditData({ 
                            ...editData, 
                            dose_per_ha: parseFloat(e.target.value)
                          })}
                        />
                        {warnings.length > 0 && (
                          <div className="absolute top-full left-0 mt-1 p-2 bg-red-50 border border-red-200 rounded shadow-lg text-left z-50 w-80 max-w-sm">
                            {warnings.map((w, i) => (
                  ...
                  <td className="px-4 py-3 text-center">
                    <div className="flex items-center justify-center gap-1 group relative">
                      <span className="text-sm text-gray-600" title={`pH před aplikací${idx > 0 ? ' (po acidifikaci)' : ''}`}>
                        {app.ph_before?.toFixed(1)}
                      </span>
                      ...
                      {idx > 0 && (
                        <button
                          ...
                          type="button"
                          title="Info o acidifikaci"
                        >
                          <Info className="w-3 h-3" />
                          <div className="invisible group-hover:visible absolute left-1/2 -translate-x-1/2 bottom-full mb-2 z-10 w-56 p-3 bg-gray-900 text-white text-xs rounded-lg shadow-lg">
                            <p className="font-semibold mb-1">💡 Přirozená acidifikace</p>
                            <p className="mb-1">
                              pH před aplikací zohledňuje přirozené okyselování půdy 
                              (~{plan.soil_type === 'L' ? '0.09' : plan.soil_type === 'S' ? '0.07' : '0.04'} pH/rok 
                              pro {plan.soil_type === 'L' ? 'lehkou' : plan.soil_type === 'S' ? 'střední' : 'těžkou'} půdu)
                            </p>
                            <p className="text-gray-300 text-[10px]">
                              Interval od předchozí aplikace: {idx > 0 ? (app.year - sortedApplications[idx - 1].year) : 0} let
                            </p>
                          </div>
                        </button>
                      )}
            ...
            {/* Projekce budoucích let PO poslední aplikaci */}
            {sortedApplications.length > 0 && (() => {
              const lastApp = sortedApplications[sortedApplications.length - 1]
              const projectionYears = 3 // Zobrazit 3 roky po poslední aplikaci
              const futureYears: Array<{year: number, phStart: number, phEnd: number}> = []
              ...
              for (let i = 1; i <= projectionYears; i++) {
                ...
                const phStart = lastApp.ph_after - (annualPhDrop * (i - 1))
                const phEnd = lastApp.ph_after - (annualPhDrop * i)
                futureYears.push({ year: yearNumber, phStart, phEnd })
              }
              
              return futureYears.map((futureYear) => (
                <tr key={`future-${futureYear.year}`} className="bg-orange-50 border-l-4 border-orange-400">
                  <td className="liming-sticky-col liming-sticky-col-1 px-4 py-2.5 bg-orange-50">
                    <span className="font-semibold text-orange-800">{futureYear.year}</span>
                  </td>
                  <td className="liming-sticky-col liming-sticky-col-2 px-4 py-2.5 bg-orange-50">
                    <span className="text-orange-600 text-sm italic">—</span>
                  </td>
                  ...
                  <td className="px-4 py-2.5 text-center">
                    <div className="flex items-center justify-center gap-1.5 text-orange-900 font-mono text-sm">
                      <span>{futureYear.phStart.toFixed(2)}</span>
                      <span className="text-orange-500">→</span>
                      <span className="font-semibold">{futureYear.phEnd.toFixed(2)}</span>
                      <span className="text-xs text-orange-600 ml-1">
                        (-{Math.abs(futureYear.phStart - futureYear.phEnd).toFixed(2)})
                      </span>
                    </div>
                  </td>
                  <td className="px-4 py-2.5">
