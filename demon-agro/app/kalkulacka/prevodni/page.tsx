...
export default function PrevorniKalkulackaPage() {
  ...
  const getConversionFactor = (nutrient: NutrientId, from: number, to: number): number => {
    if (nutrient === "Ca") {
      const forms = NUTRIENT_FORMS.Ca;
      const key = `${forms[from]}_${forms[to]}`.replace(/[₂₃]/g, (match) => 
        match === "₂" ? "2" : "3"
      );
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b-2 border-gray-200">
                  ...
                  <th className="px-4 py-3 text-left font-semibold text-gray-900">Převod</th>
